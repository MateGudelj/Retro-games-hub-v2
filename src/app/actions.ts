...
export async function createReply(formData: FormData) {
  ...
  revalidatePath(`/forum/threads/${threadId}`);
  redirect(`/forum/threads/${threadId}?success=reply-posted`);
}
...
export async function createThread(formData: FormData) {
  ...
  if (tagsString) {
    ...
    if (tagNames.length > 0) {
      ...
      if (existingTags && existingTags.length > 0) {
        ...
        const { error: threadTagsError } = await supabase
          .from("thread_tags")
          .insert(threadTags);
  ...
  // If we have a slug, redirect to the category page. Otherwise, fall back to the main forum.
  if (categorySlug) {
    revalidatePath(`/forum/${categorySlug}`);
    redirect(`/forum/${categorySlug}?success=thread-created`);
  } else {
